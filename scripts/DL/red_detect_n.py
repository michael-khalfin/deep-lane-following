...
def image_callback(ros_image): #get contour image
    ...
    global bridge, cols, state, n_laps, tot_frames, t0, perimeter,sum_steer_err

    pix_dist_to_center = 401 
...
#show the image
    ...
    if(state == 'NO_RED' and white_pct >= 0.45):

        state = 'RED'
        rospy.loginfo(f"Red Detected, Percentage: {white_pct}%")

    elif(state == 'RED' and white_pct <= 0.3):
        state = "NO_RED"
...
def jerk_cb(msg):
    global prev_velocity, prev_time, prev_velocity_derivative, count
    print("jerk_cb is called")
    # Get current time
    ...
    print(f"msg.angular.z is {current_velocity}")
    count+=1
    if count == 5:
        ...
        print(f"angular z deriv: {abs(ang_z_deriv)}")
        count = 0
...
if __name__ == '__main__':
    ...
    # rospy.Subscriber(imgtopic, Image, image_callback) #subscribes to the topic
    rospy.Subscriber('/vehicle/twist', Twist, jerk_cb)
